...
float qaGray2[] = { 0.5,0.15,0.5,1.0 };
...
void drawTyrePart2() {// 96 hinh tru
	glPushMatrix();
...
void drawHaftWheel() {
	...
	drawTyrePart2();
	//Setup Material
	setupMaterial(qaGray2, qaWhite, qaWhite, 120.0);
	//Draw
	drawRimPart1();
	glPopMatrix();
}
...
void drawBarFront(float length, float height, float radius) {
	setupMaterial(qaTyre, qaTyre, qaBlack, 60.0);
	glPushMatrix();
	glTranslatef(length*0.5, 0, 0);
	barCurveSide.DrawFaces(1);
	glPopMatrix();
...
void drawLight(float radius) {
	...
	//draw two half sphere top of light 
...
void drawFront(float length, float height, float radius) {
	...
	//ve hang rao khung cua sô phia trc
	glPushMatrix();
	glPushMatrix();
	...
	frontfrontSide2.DrawFaces(1);
	glPopMatrix();
	//draw corner - vành tròn của bản lề 
...
void drawJeepFront(float length, float width, float EdgeRadius,float sHeight, float height, float deltaHeight) {
	...
	//glRotatef(-90, 0, 0, 1);
	glRotatef(90, 0, 1, 0);
	drawBar(barLength*0.75, barHeight, barEdge);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0, barY/2, 0);
	//glRotatef(-90, 0, 0, 1);
	glRotatef(90, 0, 1, 0);
	drawBar(barLength, barHeight, barEdge);
	glPopMatrix();
	...
	glPushMatrix();
	glTranslatef(-(length + (width / 28) * 1.1 + 2 * barEdge)*1.25, barY, 0);
	//glRotatef(-90, 0, 0, 1);
	glRotatef(-270, 0, 1, 0);
	
	drawBarFront(barLength/2, barHeight, barEdge);
	glPopMatrix();
...
void drawJeepBody(float length, float width, float lHeight, float sHeight, float rimLength, float rimHeight , float edgeRadius, float edgeCurveRadius, float steepAngle) {
	...
	glDisable(GL_TEXTURE_2D);
	//wind shield, kiến chắn gió trước mặt 
	glBegin(GL_QUADS);
	...
	//draw jeep body around rims -vẽ đường bo cong vành xe
	//setupMaterial(qaTyre, qaTyre, qaBlack, 60.0);
	setupMaterial(qaGray2, qaGray2, qaWhite, 60.0);
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Bài Tập Lớn"); // open the screen window
	//create
	int nX = 4;
